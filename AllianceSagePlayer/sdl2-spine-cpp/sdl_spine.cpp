...
void CSdlSpineDrawable::Draw(float fOffsetX, float fOffsetY)
{
	...
	for (size_t i = 0; i < skeleton->getSlots().size(); ++i)
	{
		...
		for (int ii = 0; ii < pVertices->size(); ii += 2)
		{
			...
			sdlVertex.color.r = static_cast<Uint8>(tint.r * 255.f * (isAlphaPremultiplied ? tint.a : 1.f));
			sdlVertex.color.g = static_cast<Uint8>(tint.g * 255.f * (isAlphaPremultiplied ? tint.a : 1.f));
			sdlVertex.color.b = static_cast<Uint8>(tint.b * 255.f * (isAlphaPremultiplied ? tint.a : 1.f));
		...
		case spine::BlendMode_Additive:
			::SDL_SetTextureBlendMode(pSdlTexture, isAlphaPremultiplied ? SdlSpineBlendMode::AddPma : SdlSpineBlendMode::Add);
			break;
		...
		case spine::BlendMode_Screen:
			...
			break;
		default:
			::SDL_SetTextureBlendMode(pSdlTexture, isAlphaPremultiplied ? SdlSpineBlendMode::NormalPma : SDL_BLENDMODE_BLEND);
			break;
		}
